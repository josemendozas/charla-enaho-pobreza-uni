...
| Bloque | Tema | Min |
|--------|------|-----|
| 1 | La ENAHO por dentro | 12 |
| 1.5 | Manos a la ENAHO (demo en vivo) | 8 |
| 2 | De correlación a causalidad | 12 |
| 3 | Caso: agentes bancarios y remesas | 10 |
| 4 | ENAHO + datos administrativos | 8 |
...
| Año | Tasa | Contexto |
|-----|------|----------|
| 2019 | 20.2% | Pre-pandemia |
| 2020 | 30.1% | COVID-19 |
| 2021 | 25.9% | Recuperación |
| 2022 | 27.5% | Crisis política + inflación |
| 2023 | 29.0% | Ciclón Yaku + recesión |
| 2024 | 27.6% | Recuperación parcial |

10 años de progreso se perdieron en un año (2020). La pobreza aún no vuelve al nivel pre-pandemia.
...
→ Output: **Tasa de pobreza 2024: 27.8%** (cifra oficial: 27.6%)
...
for s in [1, 2, 3, 4, "5+"]:
    sub = df[df["mieperho"] == s] if isinstance(s, int) else df[df["mieperho"] >= 5]
    t = np.average(sub["pobre"], weights=sub["factor_pob"]) * 100
    print(f"{s} miembros: {t:.1f}%")
```

→ Urbano: 25.0% | Rural: 39.4% | 1 miembro: 7.2% → 5+: 40.9%
...
→ Gini: 0.348 | P10: S/288 | P50: S/641 | P90: S/1,426 | Ratio: 4.9x
...
## Ideas para perfiles de ingeniería {.smaller}

| Tema | Método | Datos |
|---|---|---|
| Cobertura de telecomunicaciones → productividad rural | DiD | OSIPTEL + ENAHO 500 |
| Infraestructura vial (km pavimentados) → empleo | DiD/IV | MTC + ENAHO 500 |
| Contaminación minera → salud en cuencas | RDD/DiD | OEFA + ENAHO 400 |
| Acceso a energía eléctrica → trabajo femenino | DiD | MEM + ENAHO 100/500 |

La ENAHO mide outcomes; los datos de ingeniería/infraestructura dan el tratamiento.
...
sumaria["gasto_pc"] = sumaria["gashog2d"] / (sumaria["mieperho"] * 12)
factor_pob = sumaria["factor07"] * sumaria["mieperho"]
stats = DescrStatsW(sumaria["gasto_pc"], weights=factor_pob)
...
Slides y código: [github.com/josemendozas/charla-enaho-pobreza-uni](https://github.com/josemendozas/charla-enaho-pobreza-uni)

jose.mendozas@pucp.edu.pe
